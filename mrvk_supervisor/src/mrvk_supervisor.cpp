...
{
    if(initialized) {
        if ((msg->twist.twist.angular.z < speed_limit) && (msg->twist.twist.angular.z > -speed_limit) && (msg->twist.twist.linear.x < speed_limit) && (msg->twist.twist.linear.x > -speed_limit)) {
            robot_stopped = true;
        } else {
            robot_stopped = false;
        }
